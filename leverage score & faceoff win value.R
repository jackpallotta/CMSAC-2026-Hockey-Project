...
wp_gam <- bam(wonGame ~ goalDifferential +
                s(secondsRemaining, by = goalDifferential, k = 8) + zoneCode + situationCode + isOT,
              data = leverageVariables, family = binomial(), method = "fREML", discrete = TRUE)

# hypothetical goal scenarios
...
# goal impact on win probability
leverageVariables <- leverageVariables |>
  mutate(currentWinProbability = predict(wp_gam,
                                         newdata = leverageVariables,
                                         type = "response"),
         
         winProbabilityGoalFor = predict(wp_gam,
                                         newdata = goalForData,
                                         type = "response"),
         
         winProbabilityGoalAgainst = predict(wp_gam,
                                             newdata = goalAgainstData,
                                             type = "response"),
...
# Faceoff Win Value Model - how much does winning vs losing matter
# conditional xG models
xGF_bam_w_l <- bam(xGFor5 ~ faceoffWon * zoneCode + goalDifferential + 
                     s(secondsRemaining, by = goalDifferential, k = 8, bs = "cr") +
                     situationCode + isOT + s(logAvailableWindow, k = 5, bs = "cr"),
                   data = leverageVariables, family = tw(link = "log"), method = "fREML", discrete = TRUE)

xGA_bam_w_l <- bam(xGAgainst5 ~ faceoffWon * zoneCode +
                     goalDifferential + s(secondsRemaining, by = goalDifferential, k = 8, bs = "cr") +
                     situationCode + isOT + s(logAvailableWindow, k = 5, bs = "cr"),
                   data = leverageVariables, family = tw(link = "log"), method = "fREML", discrete = TRUE)

# create win / loss scenarios
win_scenario <- leverageVariables |>
  mutate(faceoffWon = 1L)

loss_scenario <- leverageVariables |>
  mutate(faceoffWon = 0L)

# calculate win / loss value
leverageVariables <- leverageVariables |>
  mutate(xGF_if_win = predict(xGF_bam_w_l,
                              newdata = win_scenario,
                              type = "response"),
         
         xGF_if_loss = predict(xGF_bam_w_l,
                               newdata = loss_scenario,
                               type = "response"),
         
         xGA_if_win = predict(xGA_bam_w_l,
                              newdata = win_scenario,
                              type = "response"),
         
         xGA_if_loss = predict(xGA_bam_w_l,
                               newdata = loss_scenario,
                               type = "response"),
         
         offensiveWinValue = xGF_if_win - xGF_if_loss,
         defensiveWinValue = xGA_if_loss - xGA_if_win,
         
         netxGWinValue = offensiveWinValue + defensiveWinValue,
         
         faceoffWinValue = goalForImpact * offensiveWinValue +
           goalAgainstImpact * defensiveWinValue)

# Faceoff Leverage Model - How important is situation before puck drop
# unconditional xG models
...
# calculate pre-faceoff leverage
leverageVariables <- leverageVariables |>
  mutate(predictedxGF_5 = predict(xGF_bam, newdata = leverageVariables, type = "response"),
         predictedxGA_5 = predict(xGA_bam, newdata = leverageVariables, type = "response"),
...
# create faceoff value table
faceoffValueTable <- leverageVariables |>
  select(gameId, eventId, player, faceoffPlayerId, secondsRemaining,
         goalDifferential, zoneCode, situationCode, isOT, faceoffWon,
         # goal value
         currentWinProbability, winProbabilityGoalFor, winProbabilityGoalAgainst,
         goalForImpact, goalAgainstImpact, totalGoalSwing,
         # predicted xG before the result is known
         predictedxGF_5, predictedxGA_5,
         # leverage components
         offensiveLeverage,
         defensiveLeverage,
         rawLeverage,
         leverage,
         # conditional xG predictions
         xGF_if_win, xGF_if_loss, xGA_if_win, xGA_if_loss,
         # value of winning
         offensiveWinValue, defensiveWinValue, netxGWinValue, faceoffWinValue)

# calculate net xG created value
faceoffValueTable <- faceoffValueTable |>
  mutate(netxGCreated = faceoffWon * netxGWinValue)
...
saveRDS(faceoffValueTable, "faceoffValueData.rds")
